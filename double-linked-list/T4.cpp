...
int main() {
    ...
    for (int i = 0; i < 8; i++) {
        if (fwd[i] != a_swapped[i]) {
            cout << "Mismatch at position " << i << ": expected/correct value "
                 << a_swapped[i] << ", actual value when testing " << fwd[i]
                 << ".\n";
            return 1;
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        if (rev[i] != a_swappedRev[i]) {
            cout << "Mismatch at position " << i << ": expected/correct value "
                 << a_swappedRev[i] << ", actual value when testing " << rev[i]
                 << ".\n";
            return 1;
        }
    }
    ...
    if (preswap1_addr != postswap1_addr) {
        cout << "preswap1_addr: " << preswap1_addr << " and postswap1_addr "
             << postswap1_addr << " should be equal.\n";
        return 1;
    }
    // ASSERT_EQ(preswap1_val, postswap1_val);
    if (preswap1_val != postswap1_val) {
        cout << "preswap1_val: " << preswap1_val << " and postswap1_val "
             << postswap1_val << " should be equal.\n";
        return 1;
    }
    cout << "Passed" << endl;
    return 0;
